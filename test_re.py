...
def solution(S, pattern):
    ...
    copypattern = copy.deepcopy(pali)
    copyS = copy.deepcopy(sli)
        ...
        while sli:
            ...
            for pa in pali:
                if pa == head:
                    ...
                    break
            if previous != pali:
                previous = copy.deepcopy(pali)
                if not pali:
                    stk += 1

            else:
                pali = copy.deepcopy(copypattern)
                q = []
...
def solution(S, pattern):
    ...
    for i in range(len(S) - len(pattern) + 1):
        tmp = []
        for p in (list(permutations(pattern, len(pattern)))):
            print(S, ''.join(p), i)

            if S[i:i + len(p)] == ''.join(p) and ''.join(p) not in tmp:
                answer += 1
